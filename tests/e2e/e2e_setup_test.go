...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	//
	// 1. Initialize PCore validator nodes.
	// 2. Create and initialize PCore validator genesis files (both chains)
...
func (s *IntegrationTestSuite) runValidators(c *chain, portOffset int) {
	s.T().Logf("starting PCore %s validator containers...", c.id)
	...
	for i, val := range c.validators {
		runOpts := &dockertest.RunOptions{
			...
			Mounts: []string{
				fmt.Sprintf("%s/:/root/.persistenceCore", val.configDir()),
			},
			Repository: "persistenceone/persistencecore-e2e",
		}
		...
		s.T().Logf("started PCore %s validator container: %s", c.id, resource.Container.ID)
	}
	...
	s.Require().Eventually(
		...
		5*time.Minute,
		time.Second,
		"PCore node failed to produce blocks",
	)
}
...
func (s *IntegrationTestSuite) runIBCRelayer() {
	...
	tmpDir, err := ioutil.TempDir("", "persistenceCore-e2e-testnet-hermes-")
	s.Require().NoError(err)
	s.tmpDirs = append(s.tmpDirs, tmpDir)

	persistenceCoreAVal := s.chainA.validators[0]
	persistenceCoreBVal := s.chainB.validators[0]
	...
	s.hermesResource, err = s.dkrPool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				...
				fmt.Sprintf("PSTAKE_A_E2E_VAL_MNEMONIC=%s", persistenceCoreAVal.mnemonic),
				fmt.Sprintf("PSTAKE_B_E2E_VAL_MNEMONIC=%s", persistenceCoreBVal.mnemonic),
	...
	// create the client, connection and channel between the two PCore chains
	s.connectIBCChains()
}
